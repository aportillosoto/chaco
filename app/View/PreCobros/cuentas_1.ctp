...
<?php if (!empty($cuentas)) { ?>  
    <div class="box box-warning">
                            ...
                            <td data-title="Acciones" class="text-center">
                                <div class="btn-group">                                   
                                    <?php if ($cuenta[0]['fec_venc'] === $ctaant[0][0]['venc_min'] || empty($ctaant[0][0]['venc_min'])) { ?> 
                                        <a onclick="add()" id="<?php echo $cuenta[0]['nro_cta']."_".$consultas[0][0]['nro_pcobro']; ?>" class="btn btn-xs btn-xs btn-primary cuenta" role="button" data-title="Agregar" data-placement="top" rel="tooltip" data-toggle="modal" data-target="#mymodal"><span class="fa fa-plus"></span></a>
                                    <?php } ?> 
                                </div>
                            </td>
                        </tr>								
        ...
        <div class="box-footer">
            <a href="/sigest/pre_cobros" type="button" class="btn btn-default pull-left" data-title="Volver" data-placement="top" rel="tooltip">VOLVER</a>
...
<script>
    ...
    function add() {
        var datos = $(".cuenta").attr('id').split("_")
        $.ajax({
            type: "GET",
            url: "/sigest/pre_cobros/add_1/" +datos[0]+"/"+datos[1],
            cache: false,
            beforeSend: function () {
